...
공식 그누보드5와의 구조, 기능, 호환성 차이는 [docs/changes-from-gnuboard5.md](docs/changes-from-gnuboard5.md)에서 먼저 확인할 수 있다.

## 라이선스와 사용 조건

이 저장소는 MIT License로 배포한다. 외부 사용, 수정, 재배포 조건은 `LICENSE`와 설치 화면용 `LICENSE.txt`에 명시되어 있으며, 패키지 메타데이터도 `package.json`의 `license` 필드에 `MIT`로 표시한다.
...
- PHP 7.4.0 이상
  - 카페24 뉴아우토반 신규 신청 환경에서 제공되는 최저 PHP 버전인 7.4에 맞춘 기준이다.
  - 운영 환경은 보안 지원과 성능을 고려해 PHP 8.2 이상을 권장한다.
- MariaDB 10.x 또는 MySQL 5.7 호환 DB
  - 카페24 뉴아우토반은 MariaDB와 UTF-8 환경을 제공한다.
  - 설치 스키마는 MyISAM 기본값을 유지하지만, `config.runtime.php`의 `db_engine`으로 배포 환경의 기본 engine을 지정할 수 있다.
- PHP 확장: `mysqli`, `pdo_mysql`, `gd`, `mbstring`, `openssl`
- 관리자 회원 XLSX export ZIP 묶음을 사용하려면 `ZipArchive` 확장 필요
...
docs/changes-from-gnuboard5.md
AGENTS.md
```

## 라이선스

MIT License. 자세한 내용은 `LICENSE`와 설치 화면용 `LICENSE.txt`를 참고한다.
