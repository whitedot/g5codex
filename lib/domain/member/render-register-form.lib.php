...
class MemberRegisterFormViewDataFactory
{
    ...
    public static function build(array $request, array $member, array $config)
    {
        ...
        return array(
            ...
            'register_page_type_json' => member_json_string('register'),
            'certify_prompt_message_json' => member_json_string('회원가입을 위해서는 본인확인을 해주셔야 합니다.'),
            'password_length_message_json' => member_json_string('비밀번호를 3글자 이상 입력하십시오.'),
            'password_mismatch_message_json' => member_json_string('비밀번호가 같지 않습니다.'),
            'name_required_message_json' => member_json_string('이름을 입력하십시오.'),
